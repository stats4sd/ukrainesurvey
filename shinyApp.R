...
server <- function(input, output, session) {
  ...
  output$mymap <- renderLeaflet({
    ...
    leaflet() %>% addTiles() %>% addProviderTiles("Esri.WorldStreetMap") %>%
      setView(lng = 31.165580, lat = 48.379433, zoom = 5) %>% 
      addAwesomeMarkers(data = cluster_selected(), icon = icons, lng = 31.165580, lat = 48.379433) %>% 
      addPolygons(data = cluster_selected() , weight = 2, fillColor = "yellow", popup =paste("<h5><strong>",shapeFilter$name,"</strong></h5>",
