...
FROM
    `buildings`
GROUP BY
    `buildings`.`cluster_id`) `buildings_per_cluster` ON ((`buildings_per_cluster`.`cluster_id` = `clusters`.`id`)))
JOIN (
SELECT
...
GROUP BY
    `dwellings`.`building_id`) `dwellings_per_building` ON ((`dwellings_per_building`.`cluster_id` = `clusters`.`id`)))
GROUP BY
    `clusters`.`id`